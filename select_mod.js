document.addEventListener("DOMContentLoaded", () => {
  const initCustomSelect = (selectClass = 'select') => {
    ...
    const closeAllSelects = (except = null) => {
      document.querySelectorAll(`.${selectClass}.select_open`).forEach(select => {
    ...
    const handleOptionSelect = (select, option) => {
      ...
      selected.className = `${option.className.replace('item_select', '').trim()} item_select`;
      currentOption.textContent = selectedOption.textContent;
      
      if (prevData.caption) {
        selected.querySelector('.option_caption').textContent = prevData.caption;
      }
    ...
    selects.forEach(select => {
      ...
      select.querySelectorAll('.select_option_wrap:not(.item_select)').forEach(option => {
        option.addEventListener('click', () => handleOptionSelect(select, option));
      });
    });
